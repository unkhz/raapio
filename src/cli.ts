...
import { HtmlVisualizationPlugin, MermaidVisualizationPlugin, ChartJsVisualizationPlugin } from './plugins';
...
async function main() {
  ...
  if (command === 'analyze') {
    ...
    try {
      ...
      if (serve) {
        if (pluginName !== 'html' && pluginName !== 'chartjs') {
          logWarning(`--serve option is optimized for HTML and Chart.js plugins. Current plugin: ${pluginName}`);
        }
...
Commands:
  ...
  visualize <graphFileJson> [--output-dir <directoryName>] [--plugin <pluginName>] [--serve] [--port <number>]
    ...
    --plugin <pluginName>: Visualization plugin to use. Options: html, mermaid, chartjs.
                           Defaults to 'html'.
