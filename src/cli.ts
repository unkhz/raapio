...
import { HtmlVisualizationPlugin, MermaidVisualizationPlugin, ChartJsVisualizationPlugin, D3VisualizationPlugin } from './plugins';
...
async function main() {
  ...
  if (command === 'analyze') {
    ...
    try {
      ...
      visualizer.registerPlugin(new ChartJsVisualizationPlugin());
      visualizer.registerPlugin(new D3VisualizationPlugin());
      ...
      if (serve) {
        if (pluginName !== 'html' && pluginName !== 'chartjs' && pluginName !== 'd3') {
          logWarning(`--serve option is optimized for HTML, Chart.js, and D3.js plugins. Current plugin: ${pluginName}`);
        }
...
Commands:
  ...
  visualize <graphFileJson> [--output-dir <directoryName>] [--plugin <pluginName>] [--serve] [--port <number>]
    ...
    --plugin <pluginName>: Visualization plugin to use. Options: html, mermaid, chartjs, d3.
                           Defaults to 'html'.
...
Example:
  ...
  raapio visualize my_project_graph.json --plugin chartjs --serve
  raapio visualize my_project_graph.json --plugin d3 --serve
  `);
}
